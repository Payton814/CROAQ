...
do
rm -r ./data/$DATA_DIR$((PEAK+1)); mkdir ./data/$DATA_DIR$((PEAK+1))
cp -f ./template/dtmp.csv ./data/$DATA_DIR$((PEAK+1))/trial1.csv

echo "Sample Measurement in Progress..."
CURRENT_POS=0 ## Initialize actuator position to 0
NSTEPS=120 ## Number of times the actuator will move sample
STEP_SIZE=90 ## The step size for the actuator to make
               ## The development was done with Actuonix P8-75-165-3-ST (Each 1 step is ~0.0018mm)
python CROAQ.py $CURRENT_POS $DATA_DIR$((PEAK+1)) $PEAK ## Take sample measurement. This is the empty cavity measurement
sleep 10
python updatePlot.py ./data/$DATA_DIR$((PEAK+1))/trial1.csv &
...
do
    ...
    python CROAQ.py $CURRENT_POS $DATA_DIR$((PEAK+1)) $PEAK
    ##CURRENT_POS=$((CURRENT_POS+1))
done
